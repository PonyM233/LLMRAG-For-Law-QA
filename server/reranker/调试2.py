...
def rerank_documents(query: str, documents: Sequence[Document],
    ...
    tokenizer = AutoTokenizer.from_pretrained('/home/pc/PycharmProjects/chat0.2.4/bge-reranker-large')
    model = AutoModelForSequenceClassification.from_pretrained('/home/pc/PycharmProjects/chat0.2.4/bge-reranker-large')
    model.eval()

    with torch.no_grad():
        inputs = tokenizer(sentence_pairs, padding=True, truncation=True, return_tensors='pt', max_length=512)
        results = model(**inputs, return_dict=True).logits.view(-1, ).float()
