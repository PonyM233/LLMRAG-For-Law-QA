...
from server.chat import (twice_search ,kb_chat_2search, qa1st_then_kb, qaandlaw, chat, knowledge_base_chat, openai_chat,
                         search_engine_chat)
...
def create_app():
    ...
    app.get("/read_txt_file",
            tags=["log read"],
            summary="获取日志")(logport)
    # Tag: Chat

    app.post("/chat/twice_search",
             tags=["Chat"],
             summary="二次检索")(twice_search)

    app.post("/chat/kb_chat_2search",
             tags=["Chat"],
             summary="使用第一次的回答检索知识库")(kb_chat_2search)

    app.post("/chat/qa1st_kb2nd",
             tags=["Chat"],
             summary="先从qa找答案然后是大模型加知识库")(qa1st_then_kb)

    app.post("/chat/qaandlaw",
            tags=["Chat"],
            summary="问答对附参考法条")(qaandlaw)
